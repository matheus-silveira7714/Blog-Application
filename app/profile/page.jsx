...
const getData = async () => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_NEXTAUTH_URL}/api/profile`,
      { cache: "no-cache" }
    );
  ...
  return (
    <div className="lg:mt-3">
      <div className="flex justify-between items-center mb-2">
        ...
        <span
          onClick={signOut}
          className="w-fit px-5 py-2 softBg rounded-lg cursor-pointer"
        >
          Logout
        </span>
      </div>
      <p className="text-lg mb-3 lg:mb-4">
        ...
        <br /> Share your experiences and inspire others with your thoughts.
      </p>
      {data?.length > 0 && (
        <h2 className="font-semibold text-2xl mb-3 lg:mb-5">Recent Blogs</h2>
      )}
      {data?.length > 0 ? (
        ...
        <div className="min-h-[50vh] xl:min-h-[40vh] flex flex-col items-center justify-center gap-3 w-full mx-auto font-semibold">
          <p className="text-2xl text-center">
            You don't have any blogs in your profile.
          </p>
          <Link
            href="/write"
